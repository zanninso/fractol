...
/*   Updated: 2019/11/17 07:06:37 by aait-ihi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_argb	ft_color0(int i, int iteration, t_fractol *fractol)
{
	...
	if (BETWEEN(i, 0, x))
	{
		return (get_color(fractol->colors.c[0], fractol->colors.c[1],
		(double)(x - i) / (double)x));
	}
	if (BETWEEN(i, x, tab[0]))
	{
		return (get_color(fractol->colors.c[1], fractol->colors.c[2],
		(double)(x - i) / (double)tab[0]));
	}
	if (BETWEEN(i, tab[0], tab[1]))
		return (get_color(fractol->colors.c[2], fractol->colors.c[3],
		(double)(tab[0] - i) / (double)tab[1]));
	else
		return (get_color(fractol->colors.c[3], 0,
		(double)(tab[1] - i) / (double)tab[2]));
}
